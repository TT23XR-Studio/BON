...
function activate(context) {
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider("bon", new BonCompletionProvider(), ".", '"', "{");
    // Register hover provider for std functions
    const hoverProvider = vscode.languages.registerHoverProvider("bon", new BonHoverProvider());
    // Register definition provider (go to template/class definition)
    const definitionProvider = vscode.languages.registerDefinitionProvider("bon", new BonDefinitionProvider());
    context.subscriptions.push(completionProvider, hoverProvider, definitionProvider);
}
function deactivate() { }
// ── Completion Provider ──────────────────────────────────────
class BonCompletionProvider {
